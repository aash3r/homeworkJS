...
const result = numsArray.filter((num) => num % 2 === 0 ).reduce((a, b) => {
    return a + b;
})
...
const people = [
 ...
 const adult = people.filter(({age}) => age >= 18).map(({name}) => name)
...
const lenghtPlanet =  strings.reduce((a, b) => {
    return a.length > b.length ? a : b;
})

console.log(lenghtPlanet);
...
const newBooks = books.filter(({release}) => release === 2023).map(({title}) => title)
...
const finishNumber = Math.max(...numbers.filter((num) => num % 3 === 0))
...
const oldestPerson = people2.reduce((a, b) => {return a.age > b.age ? a : b})
