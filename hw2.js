...
const namesObject = [];
const dict = names.forEach((name) => {
    const current = {name: name.charAt(0).toUpperCase() + name.slice(1), nameLength: name.length}
    namesObject.push(current);
})
...
const seconResult = nums2.forEach((num) => {
const current = {number: num, square: num ** 2 , cube: num ** 3}
numsRes.push(current)
})
...
const thirdResult = numberRow.reverse().forEach((num) => {
reversedNumberRow.push(num)
})
...
const model = cars.forEach(({make, color, price}) => {
    console.log(`This is ${color} ${make}. It costs ${price}€.`)
})
